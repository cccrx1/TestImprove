...
from .dataset import filter_dataset_excluding_label, resolve_topk
from .log import Log
...
def test(model, dataset, schedule):
    ...
    if schedule['metric'] == 'ASR_NoTarget':
        dataset = filter_dataset_excluding_label(dataset, schedule['y_target'])
    ...
    msg = f"==========Test result on {schedule['metric']}==========\n" + \
            ...
            f"Top-1 correct / Total: {top1_correct}/{total_num}, Top-1 accuracy: {top1_correct/total_num}, Top-{top5_k} correct / Total: {top5_correct}/{total_num}, Top-{top5_k} accuracy: {top5_correct/total_num}, time: {time.time()-last_time}\n"
    log(msg)
